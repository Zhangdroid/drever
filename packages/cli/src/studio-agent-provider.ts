...
export const studioActionWorkflowInstructions = (record: DreverStudioActionRecord): string =>
  [
    ...
    record.action.type === "skip-remaining-questions"
      ...
      : record.action.type === "approve-plan"
        ? [
            ...
            "Before preview, the primary writer must not load the design skill or its references, write art-direction.md, replace Theme or Stage configuration, browse or search, or generate optional assets. Use the current coherent starter or approved surface unless a minimal complete replacement is required for safe rendering.",
            "After approval, one optional read-only visual-planning worker may run concurrently with Draft 1. It may read only the approved brief, plan, and project-local design skill to propose palette, type, Scene, and signature-moment choices. It must not write project files, browse or search for assets, or run project commands. The primary writer must not wait for it and the preview gate must not depend on it; apply any useful proposal only after preview through the single visual-source writer.",
            ...
            "Keep post-preview design iteration source-led. Inspect the existing live preview only when a concrete user-reported or high-risk question cannot be resolved from source, and then inspect the smallest affected slide, Step route, or adjacent handoff needed to answer it. Do not walk every slide or Step, or separately repeat document, speaker, and reduced-motion surfaces during design refinement; the project-local review skill owns that exhaustive rendered matrix exactly once.",
    ...
    "The project-local review skill is the single owner of final rendered evidence. Let it run `drever check --rendered --evidence .drever/review --json` once for stable source; do not manually reproduce its complete route and surface walk before or after it. Drever owns its isolated ephemeral loopback preview and Playwright browser and closes only those resources.",
  ]
