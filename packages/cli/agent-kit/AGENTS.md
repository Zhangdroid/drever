...
- Prefer the read-only `drever_get_context`, `drever_get_slide`, `drever_check`, and `drever_get_current` MCP tools when connected. Otherwise run `npm exec -- drever context --json` before substantial authoring work. These contracts describe the exact slide and Step manifest, source ranges, active design system, components, and current preflight findings.
...
- Run `npm exec -- drever check --json` and `npm exec -- drever build` after authored changes. Inspect affected exact slide and Step routes with rendered evidence. If rendered inspection is unavailable, report the gap and do not claim presentation readiness.
