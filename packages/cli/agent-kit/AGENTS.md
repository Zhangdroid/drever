...
- Prefer the read-only `drever_get_context`, `drever_get_slide`, `drever_check`, and `drever_get_current` MCP tools when connected. For an existing deck, use them before a substantial edit. For a new deck, complete the `brief.md` plus `drever.plan.json` approval gate before authoring, keep the stable plan aligned with the deck, expose the coherent Draft 1 next, then run `npm exec -- drever context --json` and the fuller checks. These contracts describe the intended story, exact slide and Step manifest, source ranges, active design system, components, and current preflight findings.
...
- After the story plan has been approved, keep one development preview alive during creation and iteration and update it through HMR. Share a coherent Draft 1 before exhaustive validation, continue refinement without another approval pause, and do not run repeated production builds merely to expose changes. Before final delivery, run `npm exec -- drever check --rendered --json` and `npm exec -- drever build`, then inspect affected exact slide and Step routes with rendered evidence. Fix machine-proven clipping, canvas overflow, overlap, scroll overflow, and resolved solid-color contrast errors; treat geometry, density, and indeterminate-paint warnings as review leads rather than aesthetic verdicts. If rendered inspection is unavailable, report the gap and do not claim presentation readiness.
