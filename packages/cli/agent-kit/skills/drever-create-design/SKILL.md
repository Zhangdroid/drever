...
- Give each moving indicator one visual role. A cursor or focus reticle has a recognizable hotspot that lands exactly on a target; a route signal remains centered on its path and reaches the real endpoint. Keep the marker and milestones in one local positioned coordinate system instead of using travel distances measured from another container.
