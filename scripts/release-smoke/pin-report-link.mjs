...
if (
  ...
  throw new Error(
    "Usage: node scripts/release-smoke/pin-report-link.mjs <pr-body> <deck-origin> <report-origin>",
  );
}
...
const immutablePagesOrigin = (value) => {
  ...
  if (
    url.protocol !== "https:" ||
    !/^[0-9a-f]{8}\.drever-website\.pages\.dev$/u.test(url.hostname) ||
    url.username !== "" ||
    url.password !== "" ||
    url.port !== "" ||
    url.pathname !== "/" ||
...
if (!source.includes(previousLink)) {
  throw new Error("Pull request body does not contain the expected release smoke report link.");
}
