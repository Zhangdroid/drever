...
`--evidence <directory>` keeps that final inspection on the same browser stack.
It writes a versioned manifest, full-size settled states, and contact sheets for
settled layouts plus forward and reverse 80 ms transition samples. Read the
manifest allowlist instead of treating unrelated files in the directory as
current evidence. No Chrome extension, Chrome MCP server, or second agent
browser is required.
