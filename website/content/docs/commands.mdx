...
| `drever dev [entry]`                   | Run the live Audience and Speaker preview.            |
...
The command starts one HMR-enabled local preview and prints Audience and Speaker
URLs. Configure `host`, `open`, `port`, and `strictPort` under `server` in
